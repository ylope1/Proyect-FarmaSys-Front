...
function agregar(){
    ...
    $("#txtlineaCred").removeAttr("disabled");
...
function listar(){
    ...
    .done(function(resultado){
        var lista = "";
        for(rs of resultado){
            lista = lista + "<tr class=\"item-list\" onclick=\"seleccionClientes("+rs.id+",'"+rs.pers_nombre+"','"+rs.pers_apellido+"',"+rs.pers_ci+",'"+rs.pers_direc+"','"+rs.pers_telef+"','"+rs.pers_email+"',"+rs.pais_id+",'"+rs.pais_desc+"',"+rs.ciudad_id+",'"+rs.ciudad_desc+"','"+rs.cli_fec_nac+"','"+rs.cli_fec_baja+"','"+rs.cli_fec_ing+"','"+rs.cli_estado+"','"+rs.cli_ruc+"','"+rs.cli_linea_credito+"');\">";
                lista = lista + "<td>";
                lista = lista + rs.id;
                lista = lista +"</td>";
                lista = lista + "<td>";
                ...
                lista = lista +"</td>";
                lista = lista + "<td>";
                lista = lista + rs.cli_fec_nac;
                lista = lista +"</td>";
                lista = lista + "<td>";
                lista = lista + rs.cli_fec_baja;
                lista = lista +"</td>";
                lista = lista + "<td>";
                lista = lista + rs.cli_fec_ing;
                lista = lista +"</td>";
                lista = lista + "<td>";
                lista = lista + rs.cli_estado;
                lista = lista +"</td>";
                lista = lista + "<td>";
                lista = lista + rs.cli_ruc;
                lista = lista +"</td>";
                lista = lista + "<td>";
                lista = lista + rs.cli_linea_credito;
                lista = lista +"</td>";
            lista = lista + "</tr>";
        }
...
function seleccionClientes(id_cli, pers_nombre, pers_apellido, pers_ci, pers_direc, pers_telef, pers_email, pais_id, pais_desc, ciudad_id, ciudad_desc, cli_fec_nac, cli_fec_baja, cli_fec_ing, cli_estado, cli_ruc, cli_linea_credito){
    $("#txtCodigo").val(id_cli);
    ...
    $("#txtFecNac").val(cli_fec_nac);
    $("#txtFecBaj").val(cli_fec_baja);
    $("#txtFecIng").val(cli_fec_ing);
    // --- MANEJO DEL SELECTPICKER ---
    $("#txtEstado").val(cli_estado);
    if (typeof $ !== "undefined" && typeof $('#txtEstado').selectpicker === "function") {
        $('#txtEstado').selectpicker('refresh');
    }
    $("#txtRuc").val(cli_ruc);
    $("#txtlineaCred").val(cli_linea_credito);
...
function grabar(){
    ...
    if($("#txtOperacion").val()==2){
        endpoint = "clientes/update/"+$("#txtCodigo").val();
        metodo = "PUT";
    }
    if($("#txtOperacion").val()==3){
        endpoint = "clientes/delete/"+$("#txtCodigo").val();
        metodo = "DELETE";
    }
    $.ajax({
        url: getUrl()+endpoint,
        ...
        data: { 
            ...
            'cli_fec_nac': $("#txtFecNac").val(),
            'cli_fec_baja': $("#txtFecBaj").val(),
            'cli_fec_ing': $("#txtFecIng").val(),
            'cli_estado': $("#txtEstado").val(),
            'cli_ruc': $("#txtRuc").val(), 
            'cli_linea_credito': $("#txtlineaCred").val(),
